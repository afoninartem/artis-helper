...
export default {
	...
	actions: {
      ...
      const lastStatus = filtered.sort((a, b) => new Date(b.updateDate).getTime() - new Date(a.updateDate).getTime())[0]
    ...
    async deleteCandidate(context, payload) {
      await db.collection("personal/hiring/candidates").doc(payload.toString()).delete()
    },
